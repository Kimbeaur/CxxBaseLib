...
int Heartbeat::send_msg(HBMSGTYPE type, const char* content, u_int len)
{
	...
	if (sendlen < 0)
	{
		printf("Heartbeat -- Socket send error! ERR:%d", errno);			
		return HB_ERR_SOCK_SEND;
	}
	printf("Heartbeat -- Send %d byte.", sendlen);	
	return HB_OK;
}
...
{
	printf("Heartbeat -- Send to IP:Port = %s:%d!", m_ip, m_port);
	while (true)
	{// 定时发送心跳
...
{
	printf("Heartbeat -- Listening IP:Port = %s:%d!", m_ip, m_port);
	...
	while (true)
	{
		...
		if (0 == ret)
		{
			printf("Heartbeat -- Recivie udp packet timeout.");
		}
		else if (ret < 0)
		{
			printf("Heartbeat -- Select socket failed! Error: %d.", errno);
			return -1;
		}
		else if (ret > 0 && FD_ISSET(m_sock, &read_set))
		{
			...
			if (recvlen < 0)
			{
				printf("Heartbeat -- recv socket failed! Error: %s\n", strerror(errno));
			}
			else if (0 == recvlen)
			{
				continue;
...
void* Heartbeat::callback(HBMSG* msg, sockaddr_in *src_sin)
{
	...
	else
	{
		...
		{
		case HB_MSG_REG:
			printf("Heartbeat -- Recv node register message!");
			printf("Heartbeat -- IP:PORT = %s:%d, descript: %s", ip, port, descript);
			...
			{
				printf("Heartbeat -- node list size greater than HB_MAX_NODE_COUNT!");
			}
			break;
		case HB_MSG_RUNNING:
			printf("Heartbeat -- Recv Heartbeat message!");
			for (int i = 0; i < n; i++)
			{
				if (node_list[i].ip == ip && node_list[i].port == port)
				{
					...
					{
						printf("Heartbeat -- Node %s:%u is dead! recv timestamp: %ll, last timestamp: %ll", 
			...
			break;
		case HB_MSG_QUIT:
			printf("Heartbeat -- Recv node exit message!");
			for (int i = 0; i < n; i++)
			{
				if (node_list[i].ip == ip && node_list[i].port == port)
				{				
...
int Heartbeat::init_sock()
{
	...
	{
		printf("Heartbeat -- Socket init error!");			
	...
	{
		printf("Heartbeat -- Invalid ip address!");
	...
	if (res < 0)
	{
		printf("Heartbeat -- Error to set reuse socket.");
	...
	if (res < 0)
	{
		printf("Heartbeat -- Error to bind socket.");
	...
	if (res < 0)
	{
		printf("Heartbeat -- Error to set socket buffer size.");
		return HB_ERR_SOCK_SETOPT;
	}
	...
	{// multicast address
		...
		if(res < 0 )
		{
			printf("Heartbeat -- Error to add membership.");
			return HB_ERR_SOCK_SETOPT;
		}
	}
...
int Heartbeat::new_thread(void* (*func)(void*), void *param)
{
	...
	if (ret >= 0) 
	{
		printf("Heartbeat -- Begin thread success!");
	}
	else
	{
		printf("Heartbeat -- ERROR: Begin thread fail! ERR:%d", ret);
	}
	return ret;
}
